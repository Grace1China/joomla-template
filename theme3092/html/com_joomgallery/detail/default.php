...
$document->addScript(JURI::base() . 'templates/'.$template->template.'/html/com_joomgallery/detail/js/jquery.validate.min.js');
$document->addScript(JURI::base() . 'templates/'.$template->template.'/html/com_joomgallery/detail/js/additional-methods.min.js');

echo $this->loadTemplate('header'); ?>
<?php if($this->_config->get('jg_showdetailtitle') == 1): ?>
  <header>
  <?php echo '<'. $template->params->get('itemItemHeading') .' class="item_title" id="jg_photo_title">'; ?>
    <?php echo wrap_with_span($this->escape($this->image->imgtitle)); ?>
...
<?php endif;
  ...
  <div class="jg_commentsarea">
    <div class="jg-details-header">
      <<?php echo $template->params->get('itemBlogItemHeading'); ?> class="jg_comments_title"><?php echo JText::_('COM_JOOMGALLERY_DETAIL_COMMENTS_EXISTING'); ?></<?php echo $template->params->get('itemBlogItemHeading'); ?>>
      </h4>
    </div>
<?php   if(!empty($this->slider)): ?>
    <div <?php echo $this->slider; ?>>
...
<?php     if(!$this->_user->get('id') && $this->_config->get('jg_namedanoncomment')): ?>
          <div class="input_wrapper"><input required type="text" class="inputbox" name="cmtname" placeholder="<?php echo $this->_mainframe->getUserState('joom.comments.name', JText::_('COM_JOOMGALLERY_COMMON_GUEST')); ?>" tabindex="1" /></div>
<?php     endif;
...
<?php     $rows = 4;
          if($this->_config->get('jg_smiliesupport')):
            ...
            <div class="input_wrapper"><textarea required cols="40" rows="<?php echo $rows; ?>" name="cmttext" class="inputbox" tabindex="100" ><?php echo $this->_mainframe->getUserState('joom.comments.text'); ?></textarea></div>
