...
<script>
	jQuery(document).bind('ready', function(){
		jQuery('#kmt-<?php echo $row->id; ?> .replyButton').click(function(){
			var intervalID = setInterval(function(){
				if(jQuery('#kmt-<?php echo $row->id; ?> #kmt-form').length){
					clearInterval(intervalID);
					jQuery('#kmt-<?php echo $row->id; ?> #kmt-form .kmt-form-submit a.replyButton').remove();
					jQuery('#kmt-<?php echo $row->id; ?> #kmt-form .kmt-form-submit').append('<a class="replyButton btn btn-primary cancel" href="javascript:void(0);"><?php echo JText::_("JCANCEL"); ?></a>')
				}
			},100)
		})
	})
